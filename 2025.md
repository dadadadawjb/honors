# 2025
* **Unified Force-Position policy** (**UniFP**)
  * title and link: [Learning a Unified Policy for Position and Force Control in Legged Loco-Manipulation](https://arxiv.org/abs/2505.20829)
  * information: CoRL 2025 best paper BIGAI (Siyuan Huang)
  * problem and position: learnable force-position controller without force sensors
  * method overview: RL in simulation to track force-position commands
  * results: tracking error within 0.1m and 10N
  * method details: 
    ![UniFP_method](assets/2025/UniFP_method.png)
    * spring-like tracking rewards in PPO
      $$
      x^{target}_{ee} = x^{cmd}_{ee} + \frac{F^{ext}_{ee} + (F^{cmd}_{ee} - F^{react}_{ee})}{K}
      $$
      $$
      v^{target}_{base} = v^{cmd}_{base} + \frac{F^{ext}_{base} + (F^{cmd}_{base} - F^{react}_{base})}{D}
      $$
    * additional external force estimation loss
    * action as joint position targets for PD controller
    * randomly sample commands and external forces to train RL in sim

* **Fabrica** (**Fabrica**)
  * title and link: [Fabrica: Dual-Arm Assembly of General Multi-Part Objects via Integrated Planning and Learning](https://arxiv.org/abs/2506.05168)
  * information: CoRL 2025 best paper MIT
  * problem and position: bimanual multi-part assembly system
  * method overview: optimization to build assembly plan, motion planning to perform general motions, RL to perform insertion motions
  * results: propose a 7-objects benchmark and achieve 80% success steps
    ![Fabrica_result](assets/2025/Fabrica_result.png)
  * method details: 
    * planning as optimization
      * assembly-hold part sequence $\phi = [o_{a, 1}, o_{h, 2}, o_{a, 2}, \ldots, o_{h, n}, o_{a, n}]$
      * grasp poses $\sigma = [g_{a, 1}, g_{h, 2}, g_{a, 2}, \ldots, g_{h, n}, g_{a, n}]$
      * robot motions $\pi = [\pi_{a, 1}^f, \pi_{a, 1}^g, \pi_{a, 1}^a, \pi_{h, 2}^f, \pi_{a, 2}^f, \pi_{a, 2}^g, \pi_{a, 2}^a, \ldots]$
      * part precedence constraint built by a previous motion planner
      * collision constraint to filter grasps
      * use dynamic programming to search the state transition tree
    * RRT-Connect plans $\pi_{a, i}^f$, $\pi_{a, i}^g$ and $\pi_{h, i}^f$, PPO in simulation for $\pi_{a, i}^a$
    ![Fabrica_method](assets/2025/Fabrica_method.png)

* **VideoMimic** (**VideoMimic**)
  * title and link: [Visual Imitation Enables Contextual Humanoid Control](https://arxiv.org/abs/2505.03729)
  * information: CoRL 2025 best student paper UCBerkeley (Trevor Darrell, Pieter Abbeel, Jitendra Malik, Angjoo Kanazawa)
  * problem and position: learn humanoid locomotion from monocular human videos
  * method overview: real2sim by reconstructing human and scene, sim2real by retargeting and RL
  * results: showcase feasibility on some demo tasks
  * method details: 
    * real-to-sim build dataset and simulation
      * detect humans by Grounded SAM 2, recover 3D SMPL parameters by VIMO and 2D keypoints by ViTPose
      * reconstruct scene by MegaSaM or MonST3R and meshify by NKSR
      * jointly optimize the human trajectory and the scene scale
      ![VideoMimic_method1](assets/2025/VideoMimic_method1.png)
    * DeepMimic with PPO to train a locomotion policy with 4 stages
      ![VideoMimic_method2](assets/2025/VideoMimic_method2.png)

* **Dexterous Universal Manipulation Interface** (**DexUMI**)
  * title and link: [DexUMI: Using Human Hand as the Universal Manipulation Interface for Dexterous Manipulation](https://arxiv.org/abs/2505.21864)
  * information: CoRL 2025 best paper finalist Stanford (Shuran Song)
  * problem and position: dexterous robot-free data collection system
  * method overview: IPhone captures wrist poses and encoders capture joint angles, inpaint exoskeleton with robot hands
  * results: showcase feasibility on some demo tasks
  * method details: 
    * design exoskeleton through optimization
      * initialize parameters $p$ as joint positions and link lengths from URDFs of robot hands
      * objective is to maximize the similarity between exoskeleton and robot's fingertip pose workspaces $\max_p S(W_{exo}^{tip}(p), W_{robot}^{tip})$, implemented by sampling configurations and finding the minimum distances
        ![DexUMI_method1](assets/2025/DexUMI_method1.png)
      * constrain parameters according to humans
      ![DexUMI_method2](assets/2025/DexUMI_method2.png)
    * inpaint exoskeleton with robot hands by SAM2 and ProPainter
      ![DexUMI_method3](assets/2025/DexUMI_method3.png)

* **Physical Intelligence 0.5** (**$\pi_{0.5}$**)
  * title and link: [$\pi_{0.5}$: a Vision-Language-Action Model with Open-World Generalization](https://arxiv.org/abs/2504.16054)
  * information: CoRL 2025 best paper finalist Physical Intelligence (Chelsea Finn, Sergey Levine)
  * problem and position: first robot foundation model to adapt in unseen scenes
  * method overview: co-training on heterogeneous tasks during pre-train
  * results: 
    ![pi05_result1](assets/2025/pi05_result1.png)![pi05_result2](assets/2025/pi05_result2.png)
  * method details: 
    * pre-train with discrete tokens by FAST on heterogeneous tasks for 280k steps
    * post-train with continuous tokens by flow matching for 80k steps
    * architecture built upon $\pi_0$
